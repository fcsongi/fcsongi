...
anvil.server.connect("server_TZ3QXHX2IWTKE5ADR7YB54M3-KOUTRITYHIAQNZVZ")
...
def worker():
    while True:
        ...
        try:
            ...
            row = app_tables.projects.get_by_id(item.getFileID())
            if row is not None:
                output = subprocess.run("node ./node_modules/webgme-engine/src/bin/run_plugin.js -b test -n CMBSD CreateProject GlobalDelivery", capture_output=True, shell=True, text=True)
                print(output.stdout)
