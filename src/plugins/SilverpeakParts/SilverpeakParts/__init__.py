...
The SilverpeakParts-class is imported from both run_plugin.py and run_debug.py
"""
import sys
import logging
import pandas
...
# Setup a logger
logger = logging.getLogger('SilverpeakParts')
...
class SilverpeakParts(PluginBase):
    def main(self):
        core = self.core
        ...
        #loads excel as json
        excel_df = pandas.read_excel('./imports/silverpeakSupport.xlsx',dtype=object)
        result = excel_df.to_json(orient="records")
        supports = json.loads(result)

        excel_df = pandas.read_excel('./imports/silverpeakAccessories.xlsx',dtype=object)
        result = excel_df.to_json(orient="records")
        ...
        commit_info = self.util.save(root_node, self.commit_hash, 'test', 'Python plugin populated the Silverpeak vendor bundles with parts')
